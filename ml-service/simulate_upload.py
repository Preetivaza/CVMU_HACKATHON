...
def simulate_upload(frontend_url="http://localhost:3000", video_path=None, gps_path=None, acc_path=None):
    ...
    try:
        with open(video_path, 'rb') as v, open(gps_path, 'rb') as g, open(acc_path, 'rb') as a:
            ...
            headers = {"Authorization": "Bearer sadaksurksha_internal_ml_service_2026"}
            response = requests.post(f"{frontend_url}/api/upload/video", files=files, headers=headers)
            end_time = time.time()
        ...
        if response.status_code == 201:
            ...
            headers = {"Authorization": "Bearer sadaksurksha_internal_ml_service_2026"}
            requests.post(f"{frontend_url}/api/v1/detections/bulk", json=detect_payload, headers=headers)
            # -----------------------------------------------------------------------------------------------
